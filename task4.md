...
2. Добавить `RabbitMQ` в docker‑compose. Объявить exchange: `account.events` (тип `topic`).
...
3.  Реализовать `Transactional Outbox` в микросервисе «Счета» для всех командных хендлеров,
    ...
    - На стороне потребителя реализовать идемпотентность (`Inbox`): таблица `inbox_consumed` (message_id UUID PK, processed_at, handler), проверять перед обработкой.
...
7. (в конце) Обновить Swagger: добавить раздел «События» с документацией по контрактам (описание полей и примеры JSON).
