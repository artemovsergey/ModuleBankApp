    ...
    : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            ...
            EventEnvelope<OutboxMessage>? @event = null;

            try
            {
                @event = JsonSerializer.Deserialize<EventEnvelope<OutboxMessage>?>(message);
                ...
                var alreadyProcessed = await db.Audit.AnyAsync(i => i.Id == @event.Payload.Id, stoppingToken);

                if (alreadyProcessed)
                {
                    log.LogInformation("Сообщение {EventId} уже обработано", @event.Payload.Id);
                }
                else
                {
                    // сохраняем в Inbox
                    db.Audit.Add(new AuditMessage(@event.Payload.Id, @event.Payload.Type, message,
                        DateTimeOffset.UtcNow));

                    // TODO: бизнес-логика обработки
                    log.LogInformation("Обрабатываю новое сообщение {EventId}", @event.Payload.Id);
            ...
            {
                log.LogError(ex, "Ошибка при обработке сообщения {EventId}", @event?.Payload.Id);
